...
def start_redis(redis_path: str) -> bool:
    ...
    for attempt in range(3):
        ...
        try:
            redis_process = subprocess.Popen([str(redis_server)], creationflags=subprocess.CREATE_NEW_CONSOLE)
...
def install_frontend_dependencies(project_root: Path, nodejs_path: str):
    ...
    node_path = Path(nodejs_path) / "node.exe"  # 仅用于验证路径
    ...
    result = subprocess.run(
        [str(pnpm_path), "install"],  # 直接执行 pnpm.cmd
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="ignore",
